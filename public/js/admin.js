...
function addQuestionCard(question) {
  ...
  card.querySelector('.qc-text').value = question && question.text ? question.text : '';

  const answers = question ? question.answers : ['', '', '', ''];
  const correctIndexes = question ? (question.correctIndexes || [question.correctIndex || 0]) : [0];

  for(let i=0; i<4; i++) {
    card.querySelector('.qc-ans-'+i).value = answers[i] || '';
    card.querySelector('.qc-chk-'+i).checked = correctIndexes.includes(i);
  }
...
async function saveQuiz() {
  ...
  for (const card of cards) {
    ...
    const questionText = card.querySelector('.qc-text').value.trim();
    
    if (!photoUrl && !questionText) { 
      errorEl.textContent = 'Chaque question doit avoir au moins un texte ou une photo.'; 
      return; 
    }

    let answersData = [];
    for(let i=0; i<4; i++) {
      const text = card.querySelector('.qc-ans-'+i).value.trim();
      const isCorrect = card.querySelector('.qc-chk-'+i).checked;
      if (text) answersData.push({ text, isCorrect });
    }

    if (answersData.length < 2) { errorEl.textContent = 'Remplissez au moins 2 réponses par question.'; return; }
    if (!answersData.some(a => a.isCorrect)) { errorEl.textContent = 'Cochez au moins une bonne réponse par question.'; return; }
    ...
    for (let i = answersData.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [answersData[i], answersData[j]] = [answersData[j], answersData[i]];
    }

    const answers = answersData.map(a => a.text);
    const correctIndexes = answersData.map((a, idx) => a.isCorrect ? idx : -1).filter(idx => idx !== -1);

    questions.push({ text: questionText, image: photoUrl, answers, correctIndexes, duration, points });
  }
